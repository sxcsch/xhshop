...
public class Page {
	...
	private String pageStrs;		//最终页面显示的底部翻页导航，详细见：getPageStr();
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			sb.append("	<li><select title='显示条数' style=\"width:75px;float:left;\" onchange=\"changeCount(this.value)\">\n");
			sb.append("	<option value=''>"+showCount+"条</option>\n");
	...
	public String getPageStrs() {
		...
		if(totalResult>0){
			if(currentPage==1){
				...
				sb.append("<span class=\"prev-disabled\"><a onclick=\"nextPage("+(currentPage-1)+")\">上页</a><b></b></span>\n");
			}
			int showTag = 5;//分页标签显示数量
			int startTag = 1;
			if(currentPage>showTag){
			...
			if(currentPage==totalPage){
				sb.append("	<a class=\"next\">下页</a>\n");
				sb.append("	<a>尾页</a>\n");
			}else{
				sb.append("	<a class=\"next\" onclick=\"nextPage("+(currentPage+1)+")\">下页</a>\n");
				sb.append("	<a onclick=\"nextPage("+totalPage+")\">尾页</a>\n");
			}
			sb.append("	<a class=\"current\">第"+currentPage+"页</a>\n");
			sb.append("<a><em>&nbsp;&nbsp;共"+totalPage+"页&nbsp;&nbsp;&nbsp;&nbsp;共"+totalResult+"条</em></a>");
	...
	public void setPageStrs(String pageStrs) {
		this.pageStrs = pageStrs;
	}
